...
### 消融结果（五档 × 三划分，实测；模型 `claude-sonnet-4-5-20250929`）
...
同一数据、同一提示词、同一种子，仅更换模型（及该端点要求的 `json_object` 模式）重跑全部五档，15 格全测：
...
- **"规则严格支配"仍未出现反例。** 换用更强模型后，依然没有任何一条案例是 LLM 对而规则错的（dev 6:0，p=0.03；holdout 与规则逐案例完全一致）。但差距从 0.25 收窄到 0.06——结论保持，余量在缩小。
- **Skills 结论更强**：dev 0.620 → 0.900（配对 33:5，p<0.0001）；challenge 弃权率仍精确等于 10 条无信号案例的占比，弃权流程在两个模型上都被执行。
- **RAG 从"无收益"变为"有安全代价"**：准确率仍无显著差异，但 `llm → llm+RAG` 的堵塞误路由率 dev 0.050 → 0.200、holdout 0.033 → 0.200，两个划分独立复现。Skills 在场时该危害归零，与知识优先级设计一致。

逐案例记录在 `evals/runs/*-deepseek.json`，与旧模型记录并存互不覆盖。两组数字不做配对比较——模型不同，`printpilot compare` 会拒绝，这是设计行为。详见 [ablation.md](evals/results/ablation.md) 末尾"跨模型复现"一节。
